...
(function() {
  ...
  this.main = function() {
    ...
    uStream.on('end', function() {
      return console.info(exec(uCode));
    });
  };
...
(function() {
  var compile, helpers, nameToJS, parse, renderJS, renderPatternJS, stdlib, withLocal, wrapInClosure, _;
  ...
  renderJS = function(node) {
    var assignment, body, clause, exprType, guard, h, i, keys, pattern, r, resultJS, statements, _i, _j, _len, _len1, _ref, _ref1, _ref2, _ref3, _ref4, _ref5;
    ...
    switch (exprType) {
      ...
      case 'def':
        if (assignment = node.assignment) {
          return renderPatternJS(assignment.pattern, renderJS(_(assignment).pick('expr')));
        }
        break;
      case 'defs':
      ...
      case 'function':
        ...
        for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
          ...
          if (pattern == null) {
            pattern = (_ref3 = node.clauses[i - 1]) != null ? _ref3.pattern : void 0;
          }
          pattern = pattern != null ? renderPatternJS(pattern, 'arg') : 'true';
          if (guard == null) {
            guard = (_ref4 = node.clauses[i - 1]) != null ? _ref4.guard : void 0;
          }
          guard = guard != null ? renderJS(guard) : 'true';
          if (!body) {
...
(function() {
  ...
  tokenDefs = [['-', /\s+/], ['-', /"[^a-z\{].*/i], ['-', /"\{.*(?:\s*(?:"|"[^\}].*|[^"].*)[\n\r]+)*"\}.*/], ['number', /\d+/], ['string', /'\(('[^]|[^'\)])*\)/], ['string', /'[^\(]/], ['string', /"[a-z][a-z0-9]*/i], ['', /(?:==|\?\{|@\{|::|\+\+|[\(\)\[\]\{\};_])/], ['dollarConstant', /\$[a-z]*/i], ['name', /[a-z][a-z0-9]*/i], ['name', /(?:<:|>:|\|:|=>|\|\||<=|>=|<>|,,|>>|<<|%%)/], ['name', /[\+\-\*:\^=<>\/\\\.\#!%~\|,&]/], ['name', /@+/]];
  ...
  this.tokenize = function(code, opts) {
    ...
    return {
      ...
      rollback: function(pos) {
        return position = _(pos).clone();
      },
      getPosition: function() {
        return _(position).clone();
      }
    };
  };
    ...
    UGrammar.prototype.getGrammar = function() {
      ...
      uGrammar = {
        rules: {
          ...
          sequence: this.seq('[', ['elements', this.optional(this.oneOrMoreWithSep(this.ref('expr'), ';'))], ']'),
...
(function() {
  var coerce, eq, k, polymorphic, round, v, _,
    __slice = [].slice;

  _ = require('../lib/underscore');

  polymorphic = function() {
    var f, fs, signatures, t;
    ...
    signatures = (function() {
      ...
      for (_i = 0, _len = fs.length; _i < _len; _i++) {
        ...
        _results.push((function() {
          ...
          _ref = ('' + f).replace(/^\s*function\s*\(([^\)]*)\)[^]+$/, '$1').split(/\s*,\s*/);
          _results1 = [];
    ...
    if (ts.length === 2) {
      ...
      switch (ts[0]) {
        ...
        case 'x':
          return [a];
        default:
          throw Error('Bad type symbol, ' + JSON.stringify(ts[0]));
      }
    } else {
